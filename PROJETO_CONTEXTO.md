...
**Score único antigo REMOVIDO (jun/2026):** `calculate_score` (singular, 0-100 agregado),
`score_color`, `INDICATOR_WEIGHTS`, `CLASS_POINTS` e `SCORE_LEVELS` foram apagados. Tudo usa
o par Qualidade × Preço. A aba Detalhe tem o painel **"Indicadores por Score"** (agrupado em
🏅 Qualidade e 💰 Preço, com a pontuação contínua e o peso de cada indicador no seu eixo); o
radar usa `score_indicator()`; a badge da sidebar mostra **Q / P**.
...
EV/EBITDA máx, **Qualidade mín, Preço mín**) + checkbox (excluir bancos). Qualidade e Preço
usam os scores novos (`calculate_scores`), aplicados localmente sobre o retorno da API.

**Filtros salvos** (persistidos no Supabase): usuário pode salvar com nome customizado.
Presets pré-configurados: "🏆 Fundamentalista", "💰 Dividendos", "🚀 Crescimento",
"🎯 Qualidade ≥ 80", "🔥 Barganhas" (Qualidade ≥ 55 e Preço ≥ 70). Filtros salvos antigos
com `score_min` são mapeados para `qual_min` (compatibilidade).
